...
from hummingbot.market.switcheo.switcheo_in_flight_order cimport SwitcheoInFlightOrder
...
cdef class SwitcheoMarketTransactionTracker(TransactionTracker):
    cdef:
        SwitcheoMarket _owner

    def __init__(self, owner: SwitcheoMarket):
        super().__init__()
        self._owner = owner
...
cdef class SwitcheoMarket(MarketBase):
    ...
    SWITCHEO_REST_ENDPOINT = "https://api.switcheo.network/v2"
    SWITCHEO_MAKER_PERCENT_FEE = 0.001
    SWITCHEO_TAKER_PERCENT_FEE = 0.002
    ...
    def __init__(self,
        ...
        self._tx_tracker = SwitcheoMarketTransactionTracker(self)
    ...
    @property
    def active_order_trackers(self) -> Dict[str, SwitcheoActiveOrderTracker]:
    ...
    @property
    def in_flight_orders(self) -> Dict[str, SwitcheoInFlightOrder]:
    ...
    def limit_orders(self) -> List[LimitOrder]:
        cdef:
            list retval = []
            SwitcheoInFlightOrder typed_in_flight_order
    ...
    def restore_tracking_states(self, saved_states: Dict[str, any]):
        self._in_flight_orders.update({
            key: SwitcheoInFlightOrder.from_json(value)
    ...
    async def _status_polling_loop(self):
        while True:
            try:
                ...
                self.logger().debug(f"Switcheo Status Polling Loop Error: {e}")
                self.logger().network(
                    "Unexpected error while fetching account and status updates.",
                    exc_info=True,
                    app_warning_msg=f"Failed to fetch account updates on Switcheo. Check network connection."
                )
    ...
    async def _update_balances(self):
        ...
        if current_timestamp - self._last_update_balances_timestamp > self.UPDATE_BALANCES_INTERVAL or len(self._account_balances) > 0:
            available_balances, total_balances = await self.get_switcheo_balances()
    ...
    async def _sequential_api_request(self,
                                      ...
                                      json: Any = None) -> Dict[str, Any]:
        """
        All trade execution calls (limit orders, market orders, cancels) on Switcheo require
        ...
        async with timeout(self.TRADE_API_CALL_TIMEOUT):
            ...
            return await self._async_scheduler.schedule_async_call(
                ...
                "Switcheo API call failed. Timeout error."
            )
    ...
    cdef object c_get_fee(self,
                          ...
                          object price):
        cdef:
            int gas_estimate = 140000  # approximate gas used for Switcheo market orders
            double transaction_cost_eth

        if order_type is OrderType.LIMIT:
            return TradeFee(percent=self.SWITCHEO_MAKER_PERCENT_FEE)
    ...
    #         ["expires", 100000, "uint256"],  # expires does nothing but is required according to Switcheo documentation
    #         ["nonce", nonce, "uint256"],
    ...
    #             "amount": str(int(amount_quote)),  # amount is required to be a string (Switcheo API bug)
    ...
    #         vrs = generate_vrs(hash_data, self.wallet.private_key)
    #         market_orders.append({
    #             "amount": str(int(amount_base)),  # amount is required to be a string; Switcheo API bug
    #             "orderHash": o["orderHash"],
    #             "address": self._wallet.address,
    #             "nonce": nonce,
    #             "v": vrs["v"],
                ...
                if tracked_order is not None:
                    ...
                    self.c_trigger_event(self.MARKET_BUY_ORDER_COMPLETED_EVENT_TAG,
                                         BuyOrderCompletedEvent(timestamp=self._current_timestamp,
                                                                ...
                                                                fee_amount=float(
                                                                    fill_quote_amount * self.SWITCHEO_TAKER_PERCENT_FEE
                                                                ),
    ...
    async def cancel_all(self, timeout_seconds: float) -> List[CancellationResult]:
        ...
        try:
            ...
            self.logger().network(
                ...
                exc_info=True,
                app_warning_msg=f"Failed to cancel orders on Switcheo. Check Ethereum wallet and network connection."
            )
