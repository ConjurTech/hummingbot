...
class SwitcheoTime:
    """
    Used to patch Switcheo client's time module to adjust request timestamp when needed
    """
    SWITCHEO_TIME_API = "https://api.switcheo.network/v2/exchange/timestamp"
    _bt_logger = None
    ...
    @classmethod
    def get_instance(cls) -> "SwitcheoTime":
        if cls._bt_shared_instance is None:
            cls._bt_shared_instance = SwitcheoTime()
        ...
        while True:
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.get(self.SWITCHEO_TIME_API) as resp:
                        time_now_ms = time.time() * 1e3
                        resp_data = await resp.json()
                        switcheo_server_time = resp_data["serverTime"]
                        time_after_ms = time.time() * 1e3
                expected_server_time = int((time_after_ms + time_now_ms)//2)
                time_offset =  switcheo_server_time - expected_server_time
                self.set_time_offset_ms(time_offset)

            except asyncio.CancelledError:
                raise
            except Exception:
                self.logger().network(f"Error getting Switcheo server time.", exc_info=True,
                                      app_warning_msg=f"Could not refresh Switcheo server time. "
